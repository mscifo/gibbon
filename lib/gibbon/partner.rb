module Gibbon
  class Partner < APICategory
    ...
    def export_api_url
      "http://#{get_data_center_from_api_key}partner-api.mailchimp.com/1.3/"
    end
    ...
    def set_instance_defaults
      super
      @app_key = self.class.app_key if @app_key.nil?
    ...
    class << self
      ...
      def method_missing(sym, *args, &block)
        new(self.app_key, self.api_key, {timeout: self.timeout, throws_exceptions: self.throws_exceptions}).send(sym, *args, &block)
      end
    end
  end
end